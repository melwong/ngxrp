...
function qa_show_waiting_after(elem, inside) {
	if (elem && !elem.qa_waiting_shown) {
		...
		if (w) {
			...
			//Mel: Disable element like button to prevent clicking while waiting
			//elem.disabled = true;
...
function qa_hide_waiting(elem) {
	...
	if (c) {
		...
		//Mel: Turn back on element after waiting is over
		//elem.disabled = false;
...
async function qa_submit_wall_post(elem, morelink) {
	...
	if (amount_left >= params.minbalance) {
		...
		await withdrawCoins(params.message, params.wallet).then(function (txhash) {
			params.txhashurl = qa_html_escape('https://testnet.xrpl.org/transactions/' + txhash + '');
	...
	window.location.reload();		//Mel: Refresh withdrawal page to get new balance after withdrawal.
