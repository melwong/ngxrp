...
if ($start == 0) { // only allow posting on first page
	if ($wallposterrorhtml) {
		...
		$qa_content['message_list']['form']['fields'] = array(
			'message' => array(
				'tags' => 'name="message" id="message" type="number" placeholder="' . qa_lang('profile/wall_form_placeholder') . $maxwithdrawalamount . '" required',	//Mel: Allow only numeric entry into form
				//'tags' => 'name="message" id="message"',
				'value' => qa_html(@$inmessage, false),
				'rows' => 1,		//Mel: Reduce height
				//'rows' => 2,
				'error' => qa_html(@$errors['message']),
			),

			//Mel: Add hidden field containing coins/points balance
			'balance' => array(
				'tags' => 'name="balance" type="hidden"',
				'value' => qa_db_get_coins_balance($loginuserid),
			),

			//Mel: Add hidden field containing min coins balance
			'minbalance' => array(
				'tags' => 'name="minbalance" type="hidden"',
				'value' => qa_opt('points_base'),
			),

			//Mel: Add hidden field containing wallet address
			'coins' => array(
...
foreach ($usermessages as $message) {
	...
	//Mel: To hide the withdrawal wall list from other users except for the user who owns the wall
